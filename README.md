...
```mermaid
flowchart TD
    subgraph L5["L5 — umbrella"]
        direction LR
        scitex["scitex"]
    end
    subgraph L4["L4 — observers / UX"]
        direction LR
    ...
    subgraph L3["L3 — workflow producers"]
        direction LR
        ...
        scitex_todo["scitex-todo"]
        scitex_ui["scitex-ui"]
        scitex_web["scitex-web"]
        scitex_writer["scitex-writer"]
        socialia["socialia"]
    end
    subgraph L2["L2 — domain producers"]
        direction LR
        figrecipe["figrecipe"]
    ...
    subgraph AX["Orthogonal — orchestration / platform"]
        direction LR
        newb["newb"]
    ...
    %% same-layer optional peers (illustrative)
    figrecipe <-. optional .-> scitex_stats

    %% Orthogonal touches every layer but is not part of the cascade
    AX -. orchestrates / hosts .-> L5
```
